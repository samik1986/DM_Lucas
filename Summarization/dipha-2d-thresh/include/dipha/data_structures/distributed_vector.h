...
namespace dipha
{
  namespace data_structures
  {
    ...
    protected:
      std::map< int64_t, value_type > entries;
    ...
    public:
      ...
      void set_local_value(int64_t idx, const value_type& value)
      {
        ...
        keys.push_back(idx - local_begin);
      }

      value_type get_local_value(int64_t idx) const
      {
        assert(idx >= local_begin && idx < local_end);
        return entries.at(idx - local_begin);
      }

      value_type get_local_value_ind(int64_t) const
      {
                             ...
                             std::vector< value_type >& values) const
      {
        ...
        for (int source = 0; source < mpi_utils::get_num_processes(); source++)
        {
          for (const auto& query : queries_buffer[source])
            answers_buffer[source].push_back(entries.at(query - local_begin));
        }
      ...
      void set_global_values(const std::vector< std::pair< int64_t, value_type > >& indices_and_values)
      {
        ...
        //std::cout << "processes: " << mpi_utils::get_num_processes() << std::endl;
        for (const auto& index_and_value : indices_and_values)
        {
          //std::cout << index_and_value.first << " " << index_and_value.second << std::endl;
          //std::cout << element_distribution::get_rank(global_num_elems, index_and_value.first) << std::endl;
        ...
        //std::cout << "receive pairs" << std::endl;
        ...
        for (int idx = 0; idx < mpi_utils::get_num_processes(); idx++)
        {
          ...
          for (const auto& buffer_elem : buffer)
            {
            //std::cout << buffer_elem.first << " " << buffer_elem.second << std::endl;
      ...
      std::vector<int64_t> get_keys() const
      {
        return keys;
      }
    };
