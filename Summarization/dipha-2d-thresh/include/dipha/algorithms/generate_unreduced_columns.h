...
namespace dipha
{
  namespace algorithms
  {
                                    ...
                                    data_structures::flat_column_stack& unreduced_columns)
    {
      ...
      //std::vector<int64_t> valid_cells = cell_to_filtration_map.get_keys();
      int64_t filt_size = filtration_to_cell_map.get_size();

      const int64_t num_chunks = (filt_size + globals::DIPHA_BLOCK_SIZE - 1) / globals::DIPHA_BLOCK_SIZE;
      ...
      {
        //std::cout << "chunk idx: " << chunk_idx << " / " << num_chunks << std::endl;
        const int64_t chunk_begin = (filt_size - 1) - chunk_idx * globals::DIPHA_BLOCK_SIZE;
        const int64_t chunk_end = std::max((filt_size - 1) - (chunk_idx + 1) * globals::DIPHA_BLOCK_SIZE, local_begin);
