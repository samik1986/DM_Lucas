...
namespace dipha
{
  namespace algorithms
  {
                                           ...
                                           data_structures::distributed_vector< int64_t >& cell_to_filtration_map)
    {
      ...
      for (int64_t idx = local_begin; idx < local_begin + filtration_to_cell_map.get_size(); idx++)
        indices_and_values.push_back(std::make_pair(filtration_to_cell_map.get_local_value(idx), idx));
      std::cout << "index and value size: " << indices_and_values.size() << std::endl;
      cell_to_filtration_map.init(indices_and_values.size());
